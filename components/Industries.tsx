...
function IndustryTile({
  ...
  return (
    <div className="flex flex-col items-center gap-3.5 px-1">
      <div className="relative">
        <span
          aria-hidden="true"
          className="absolute inset-0 rounded-[22px] opacity-40 blur-xl"
          style={{ background: `var(--color-${state})` }}
        />
        <div className="relative flex size-[82px] items-center justify-center rounded-[20px] bg-paper p-[22px] shadow-[0_18px_40px_-14px_rgb(0_0_0/0.55)]">
          <span style={{ color: `var(--color-${state})` }}>{icon}</span>
        </div>
      </div>
      <span className="whitespace-nowrap rounded-full bg-paper/95 px-3.5 py-1.5 text-[0.8rem] font-medium text-ink shadow-sm">
        {name}
      </span>
    </div>
  );
...
export default function Industries() {
      ...
      mm.add(
        ...
        (ctx) => {
          if (ctx.conditions?.reduced) {
            gsap.set(".ind-head, .ind-loop", { opacity: 1, y: 0 });
            return;
          }
          gsap.from(".ind-head", {
            y: 26,
          ...
          gsap.from(".ind-loop", {
            ...
            duration: 0.9,
            ease: "power3.out",
            scrollTrigger: { trigger: ".ind-loop", start: "top 88%", once: true },
          });
        }
      );
      return () => mm.revert();
  ...
  const items: LogoItem[] = useMemo(
    () =>
      INDUSTRIES.map((i) => ({
        node: <IndustryTile name={i.name} state={i.state} icon={i.icon} />,
        title: i.name,
        ariaLabel: i.name,
      })),
    []
      ...
      <div className="relative py-24 sm:py-32">
        ...
        <div className="ind-loop mt-16">
          <LogoLoop
            ...
            fadeOut
            fadeOutColor="#101826"
            ariaLabel="Industries we deliver for"
          />
        </div>
      </div>
    </section>
