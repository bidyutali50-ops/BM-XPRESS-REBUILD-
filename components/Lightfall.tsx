...
export default function Lightfall({
  ...
  // Serialise the colors array so an inline `[...]` at the call site
  // doesn't retrigger the effect on every parent render (a real bug in
  // the source: arrays are reference-unstable, useEffect deps would
  // rebuild the WebGL context every re-render).
  ...
  useEffect(() => {
    ...
    // Reduced motion: don't init WebGL at all. Container stays present
    // but transparent, so any static bg colour behind it shows through.
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        cleanup = init(ogl, container);
      } catch (err) {
        // eslint-disable-next-line no-console
        console.warn("Lightfall FX failed:", err);
      }
    })();

    function init(ogl: typeof import("ogl"), host: HTMLDivElement) {
      ...
      // Pause the RAF when the section leaves the viewport. Real perf
      // win — the source runs forever even when scrolled past.
      let visible = true;
      let raf = 0;
      let lastTime = 0;
      ...
      return () => {
        ...
        try {
          ...
          /* ogl versions differ on cleanup shape */
        }
      };
    }
