...
export default function SpecularButton({
  ...
  useEffect(() => {
    ...
    // Skip WebGL entirely under reduced motion — the button stays a plain
    // styled control, no canvas, no RAF loop.
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        cleanup = init(ogl, btn, fx);
      } catch (err) {
        // WebGL / ogl failed — the button still works, just without the FX
        // eslint-disable-next-line no-console
        console.warn("SpecularButton FX failed:", err);
      }
    })();
  ...
  function init(
    ...
    // Pause RAF when the button is offscreen. Real perf win vs the source,
    // which keeps the WebGL loop running forever.
