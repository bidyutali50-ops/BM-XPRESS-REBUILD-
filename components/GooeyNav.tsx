...
/**
 * Pill nav with a gooey particle burst on click.
 * Original: React Bits (MIT). This version is TS + tuned to the site palette.
 */
export default function GooeyNav({
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      window.setTimeout(() => {
        ...
        particle.style.setProperty("--color", `var(--gnav-c${p.color}, white)`);
        ...
        window.setTimeout(() => {
          try {
            ...
            // particle may have already been cleared on re-click
          }
        }, t);
      }, 30);
    }
  ...
  const updateEffectPosition = (target: HTMLElement) => {
  ...
  const activate = (target: HTMLElement, index: number) => {
    if (activeIndex === index) return;

    setActiveIndex(index);
    updateEffectPosition(target);

    // Skip the burst if reduced motion is set
    ...
    if (textRef.current) {
      ...
      void textRef.current.offsetWidth; // force reflow so animation restarts
  ...
  useEffect(() => {
    ...
    const activeAnchor = navRef.current
    ...
    const ro = new ResizeObserver(() => {
      const target = navRef.current
        ?.querySelectorAll<HTMLAnchorElement>("li > a")
        [activeIndex];
      if (target) updateEffectPosition(target);
    });
  ...
  return (
    <div className="gooey-nav-container" ref={containerRef}>
      <nav>
        <ul ref={navRef}>
          {items.map((item, i) => (
            <li key={item.href} className={activeIndex === i ? "active" : ""}>
